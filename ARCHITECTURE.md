...
Immediately after the const rewrite, the same join body calls `kermit_algos::rewrite_repeated_variables` (see `kermit-algos/src/selection_rewrite.rs`). WCOJ algorithms assume each variable occurs at most once per atom — one trie level binds one variable — and `Q(X) :- r(X, X).` breaks that: both executors registered `r` once for `X`, so its second column was never opened (LFTJ returned every subject, HashTriejoin panicked at the leaf). The rewrite is relational algebra's answer, a selection: every second-and-later occurrence of a variable within one body atom becomes a fresh body-only variable `K<i>`, and the atom is renamed to a synthetic `Select_<n>_<base>` predicate carrying the column equalities `σ_{col_i = col_j}`. `JoinFamily::wrap_selection` backs it with a non-materialised view — `EqualitySelectionTrieIter` (sorted) or `EqualitySelectionHashTrieIter` (hash) — that delegates every level to the base relation's iterator except the constrained ones, where it admits only the key (or hash, plus a value filter on the leaf chain) it passed at the source level. `r(X, X), r(X, Y)` yields two views of `r` because the selected atom is renamed. Const first, then selection: the second pass sees an atom-free body, and both draw fresh names from one `K<n>` counter. Head repeats (`Q(X, X)`) are not rewritten. Neither executor changed.
...
6. **Wire the tests.** Add `define_multiway_join_test_suite!(<Type>, <Algo>, LexicographicOptimiser)` and a second invocation with `CardinalityOptimiser` in `kermit/tests/join_tests.rs`, so all 14 standard join patterns run against the structure. Each distinct layout combination is its own suite invocation (e.g. `HashTrieSip`, `HashTrieFx`).
