...
- **Hash function consistency.** `hash_attribute(depth, key)` is the single source of truth for the hashing convention, exported from `kermit-iters`. It ignores the `depth` parameter and hashes only the key value. The depth parameter is preserved in the signature for ABI compatibility with the original design but does not affect the output. Cross-attribute aliasing isn't an issue because attribute positions live in physically distinct hash tables — a value at column 0 and the same value at column 1 are stored in different tables and cannot collide. Both `HashTrie::insert_at` and `SingletonHashTrieIter::new` import this function; any divergence silently breaks queries against constants.
...
- **Leaf chains preserve hash collisions.** Two tuples with identical hash signatures (collisions on every attribute) end up in the same leaf chain. Verification at join time (paper §3.2.3 line 18) distinguishes true matches from false positives.
