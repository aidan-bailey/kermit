...
cargo run -- bench run --all -i tree-trie -a leapfrog-triejoin     # Every benchmark, one valid (structure, algorithm) pair
# NOTE: -i all / -a all does NOT filter incompatible pairs — `-a all` panics, `-i all` mislabels HashTrie; enumerate the 3 valid pairs (see Gotchas).
...
4. **Implement `TrieIterable`** for the structure (wires `trie_iter()` to your iter type). The LFTJ `open`-after-`at_end` discipline is load-bearing — see the LFTJ gotcha and the `feedback_lftj_open_after_at_end` memory.
   For a **hash-family** structure, implement `HashTrieIterator` + `HashTrieIterable` instead (`#[derive(IntoTrieIter)]` does not apply). Be aware that the shared helpers — `project_via_trie_iter`, `TrieIteratorWrapper`, and every `kermit-ds` macro test suite — are `TrieIterable`-only, so you must supply your own equivalents; see the test-macro gotcha.
...
6. **Wire the CLI.** Add a variant to `IndexStructureSelector` in `kermit/src/main.rs` and to its `expand()` method. Add match arms in `run_ds_bench` and `run_benchmark` in the same file (or `run_ds_bench_hash` / `run_benchmark_hash` for a hash-family structure).
...
4. **Wire the CLI.** Add a variant to `JoinAlgorithmSelector` in `kermit/src/main.rs` and to its `expand()` method. A **sorted-family** algorithm is also wired into `instantiate_database` (`kermit/src/db.rs`); a **hash-family** algorithm needs its own execution path instead, as `hash_join` is for `HashTriejoin` — a second `DB` impl overlaps the existing one and fails coherence (E0119).
...
- **`bench run -i all` / `-a all` is broken — enumerate valid pairs**: the cross-product loop in `run_bench_run_command` does **not** filter incompatible (structure, algorithm) pairs, and the up-front `supports_algorithm` gate is permissive whenever either selector is `All`. `-a all` panics on e.g. `(TreeTrie, HashTriejoin)` (`instantiate_database` in `kermit/src/db.rs`) and writes no report. Worse, `-i all -a leapfrog-triejoin` does **not** panic — `dispatch_run_bench` routes on the structure alone, reaches the `HashTrie` arm, runs `hash_join` regardless of the algorithm, and stamps the report's algorithm axis `LeapfrogTriejoin` (silent mislabel, in `run_benchmark_hash`). Note the unit test `supports_algorithm_filters_incompatible_pairs` currently *asserts* the permissive behaviour, so a fix must update it too. Only three pairs are valid — `(tree-trie|column-trie, leapfrog-triejoin)` and `(hash-trie, hash-triejoin)` — so sweep by running one `bench run` per pair. Filtering the loop is a filed known-issue.
- **`kermit-ds` test macros are `TrieIterable`-only**: `relation_trie_test_suite!`, `trie_traversal_tests!`, `trie_seek_tests!` and `parquet_test_suite!` (`kermit-ds/tests/common/macros.rs`) all bound on `TrieIterable`/`TrieIterator`, so a hash-family structure **cannot** be plugged into them — `HashTrie` has zero presence in `kermit-ds/tests/`. It is covered at the join level only, via `kermit/tests/join_tests.rs` (`HashTrieSip`/`HashTrieFx`). Consequence: adding a hash-family index structure means hand-writing a `HashTrieIterator` contract suite (open/next/lookup/up/leaf_tuples) and a `collect_tuples()`-driven Parquet round-trip; Priorities item 1 is not satisfiable through the existing macros alone.
