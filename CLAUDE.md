...
- **Space benchmarks**: `kermit/src/measurement.rs` contains `SpaceMeasurement` (custom Criterion `Measurement`) and `BytesFormatter`. Both `bench ds --metrics space` and `bench run --metrics space` route through `Criterion<SpaceMeasurement>` via `iter_custom`, producing `target/criterion/` output alongside the time metrics. The closure reconstructs the relation per iter and sums `heap_size_bytes()` so Criterion's calibration phase observes real work; the result is deterministic (zero-variance), so per-iter mean equals one `heap_size_bytes()` exactly. Plots are disabled via `.without_plots()` because Criterion's plotters backend panics on zero-variance data — see `build_space_criterion` in `kermit/src/main.rs`.
- **JSON bench reports**: `kermit bench --report-json <PATH>` writes a machine-readable report describing one invocation. Output is always a JSON array of `BenchReport` objects (one per query for `bench run`, exactly one for `bench join` / `bench ds`). Each object carries the same `metadata` shown on stderr plus `criterion_groups` pointers that resolve to `target/criterion/{group}/{function}/`. Schema is versioned via `schema_version` (currently `1`) and lives in `kermit/src/bench_report.rs`. Bump the version on any breaking field-name or value-type change.
