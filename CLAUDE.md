...
cargo run -- bench run triangle -i hash-trie -a hash-triejoin --ds-config singleton-pruning=true  # Config axis
...
- **ConfigurableRelation**: `Relation` + a runtime Config value (`with_config` / `from_tuples_with_config` / `config()`, in `kermit-ds/src/relation.rs`); implemented only by `HashTrie<H>`. `Configured<R, P>` (`kermit-ds/src/configured.rs`) lifts a config value to a type so macro test suites can name it.
...
- `define_multiway_join_test_suite_with_config!(Relation, Algo, Optimiser, Provider)` — the same 11 patterns on `Configured<Relation, Provider>`, one module per invocation; pair it with `kermit_ds::define_config_provider!` to name the config value (see `PruningOn` in `kermit/tests/join_tests.rs`)
