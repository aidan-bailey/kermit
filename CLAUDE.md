...
- **WatDiv benchmark generation**: the 12 `watdiv-stress-*.yml` files are produced by `scripts/watdiv-preprocess/` (Python). Editing them by hand drifts from the preprocessor; regenerate instead. The integration test at `kermit/tests/watdiv_correctness.rs` loads a committed mini fixture — no Python required at test time.
