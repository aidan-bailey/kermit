...
- **No Criterion auto-plots**: `kermit/Cargo.toml` opts out of Criterion's default features (`default-features = false, features = ["rayon", "cargo_bench_support"]`) so the `plotters` dep is excluded entirely. Result: no SVG/HTML rendering, and the zero-variance panic that `SpaceMeasurement` used to trigger no longer applies. Measurement JSON (`estimates.json`, `sample.json`, `benchmark.json`, `tukey.json`) is still written per-function under `target/criterion/{group}/{directory_name}/{base,new}/`. Plot generation lives in `python/kermit-lab/` (uv-managed Python project, matplotlib + seaborn) — see `docs/specs/2026-05-04-remove-criterion-graphs-design.md`.
