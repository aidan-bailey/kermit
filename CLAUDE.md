...
1. **Test coverage for every algorithm × index-structure pair.** New algorithms and index structures must extend `define_multiway_join_test_suite!` (see Testing Patterns) so the 14 standard join patterns run against every combination. A change that adds a structure or algorithm without extending the suite will be rejected. Each Layout combination on a DS is a distinct test variant — for `HashTrie` that is now hasher × pruning, i.e. the four aliases `HashTrieSip`, `HashTrieFx`, `HashTrieSipPruned`, `HashTrieFxPruned`; each must run the 14 standard join patterns under every compatible algorithm and both optimisers. Config dimensions are tested via `define_multiway_join_test_suite_with_config!` (baseline + one alternate per value; see `kermit/tests/join_tests.rs` for the `HalfFull` load-factor precedent). BuildMode is tested via the prescribed (not-yet-implemented) `define_multiway_join_test_suite_for_build_mode!`; until the first BuildMode consumer lands, write per-mode tests by hand. See [`docs/specs/optimization-standard.md`](docs/specs/optimization-standard.md).
...
cargo run -- bench run triangle -i hash-trie -a hash-triejoin --ds-layout-pruning on  # Pruning Layout axis
cargo run -- bench run triangle -i hash-trie -a hash-triejoin --ds-config load-factor=0.5  # Config axis
cargo run -- bench run triangle -i all -a all -m iteration --verify  # Check answer counts against the YAML's expected before timing
...
kermit-rdf      → RDF/SPARQL preprocessing pipelines for on-the-fly benchmark generation.
                  ...
                  `sparql::translator`, `yaml_emit` (`yaml_emit` writes each
                  query's `expected` cardinality when the generator knows it).
...
kermit          → CLI binary (clap). Subcommands: join, bench (join|ds|run|list|fetch|clean|
                  ...
                  All Criterion execution lives here (in `src/bench/`; including SpaceMeasurement).
```
...
- **QueryOptimiser**: plans a `QueryPlan` (the LFTJ global attribute order) from a query + `CatalogStats` (`optimiser/stats.rs`); consumed by `JoinAlgo::join_iter`. `QueryPlan` is the only type crossing planner → executor; the canonical variable numbering both sides agree on lives one level up in `kermit-algos/src/analysis.rs`. Implementations: `LexicographicOptimiser` (default), `CardinalityOptimiser`.
- **ConfigurableRelation**: `Relation` + a runtime Config value (`with_config` / `from_tuples_with_config` / `config()`, in `kermit-ds/src/relation.rs`); implemented only by `HashTrie<H, P>` (its Config value is the load-factor cap). `Configured<R, P>` (`kermit-ds/src/configured.rs`) lifts a config value to a type so macro test suites can name it.
...
- `define_multiway_join_test_suite!()` — generates 14 standard join patterns (unary, triangle, chain, star, self-join, existential, empty-result, single-relation, four-way-chain, wide-fanout, dead-end, deep-dead-end, diagonal, repeated-nonadjacent). The suite runs through the real entry points (`lftj_join` / `hash_join` via `kermit/tests/common/utils.rs::JoinEntry`), so both rewrites are on the tested path; rows are projected to the head before comparison
- `define_multiway_join_test_suite_with_config!(Relation, Algo, Optimiser, Provider)` — the same 14 patterns on `Configured<Relation, Provider>`, one module per invocation; pair it with `kermit_ds::define_config_provider!` to name the config value (see `HalfFull` in `kermit/tests/join_tests.rs`, and `NinetyPercent` in `kermit-ds/tests/hash_trie_tests.rs`)
...
1. **Module layout.** `kermit-algos/src/` is split by *iterator family*: `sorted/` holds everything traversing `TrieIterable`, `hash/` everything traversing `HashTrieIterable`, and the two never reference each other. Create `kermit-algos/src/<family>/<name>.rs`, naming the file to match its `docs/algorithms/<name>.md` page and its CLI value. Existing precedents: `kermit-algos/src/sorted/leapfrog_join.rs` (binary intersection, internal helper), `kermit-algos/src/sorted/leapfrog_triejoin.rs` (multi-way join, the CLI-exposed entry point), and `kermit-algos/src/hash/hash_triejoin.rs`. Each family directory also holds a `singleton.rs` (the const-view rewrite's one-value relation), a `selection.rs` (the selection rewrite's equality view for repeated variables) and an `iter_kind.rs` (the real-relation/singleton/selection wrapper); an algorithm in a *new* family needs all three alongside it, plus a `mod.rs` documenting the family contract.
2. **Implement `JoinAlgo<DS>`**, narrowing `DS` to the iterator family you actually traverse — `TrieIterable` (as `LeapfrogTriejoin` does) or `HashTrieIterable` (as `HashTriejoin` does). The trait's own bound is the weak `JoinIterable` marker. The algorithm must tolerate const-rewritten queries (extra unary `Const_c<id>` body predicates from `kermit_algos::rewrite_atoms`) and selection-rewritten ones (`Select_<n>_<base>` predicates from `kermit_algos::rewrite_repeated_variables`, which guarantee no variable repeats within an atom) — see the two rewrite gotchas — and must validate the incoming `QueryPlan`.
3. **Register the module.** Add `mod <name>;` and `pub use <name>::<Type>;` in the family's `kermit-algos/src/<family>/mod.rs`, re-export it from the flat facade in `kermit-algos/src/lib.rs`, and add a variant on the `JoinAlgorithm` enum in that file. The facade is deliberately flat — consumers name `kermit_algos::HashTriejoin`, never the family path.
4. **Wire the CLI.** Add a variant to `JoinAlgorithmSelector` in `kermit/src/main.rs` and to its `expand()` method, then add the algorithm's valid cells to `Execution` / `Execution::for_pair` in `kermit/src/execution.rs` (the compiler flags the incomplete match) and a `dispatch_run_bench` arm in `kermit/src/bench/run.rs`. `kermit join` / `bench join` dispatch through the same `Execution` cells (`load_query_runner` in `main.rs`), so a cell added there is reachable from every command. A sorted-family algorithm plugs into `lftj_join<R, JA>` (generic in the algorithm); a hash-family one needs its own entry point beside `hash_join`, which hardwires `HashTriejoin` — both are thin wrappers over the shared private body in `kermit/src/db.rs`, parameterised by the `JoinFamily` trait.
...
4. **Wire the CLI.** Add a variant to the `Optimiser` enum in `kermit-algos/src/optimiser/mod.rs`, beside the `QueryOptimiser` trait it names (with `instantiate()` and `axis_value()` arms; the `axis_values_match_clap_value_names` guard test pins axis naming). The `--optimiser` flag on `join`, `bench join`, and `bench run` picks it up via `ValueEnum`.
5. **Wire the tests.** Two hooks, both required. *Breadth:* add a `define_multiway_join_test_suite!(<DS>, <Algo>, <YourOptimiser>);` invocation per valid (DS, algorithm) pair in `kermit/tests/join_tests.rs` (Priorities item 1), plus unit tests for the ranking itself in your module. *Depth:* `kermit/tests/lubm_mini_oracle.rs` (runs in CI, Java-free) iterates `Optimiser::value_variants()`, so your variant is checked against its hand-counted 14-query cardinalities with no edit; also add a row to the `optimisers` vec in `kermit/tests/lubm_cardinalities.rs` so the policy is checked against the published LUBM(1, 0) reference cardinalities (real LUBM(1, 0) data; needs `java` on PATH, so run it inside `nix develop` — it skips silently otherwise). The breadth suite is combinatorial but its 3-5 tuple fixtures never prune deeply enough to fail a trie descent, so it cannot see an executor bug that only some variable orderings expose; `lubm_cardinalities.rs` is the only test with real data and is where such a bug surfaces. Skipping the second hook is how a wrong-answer LFTJ bug survived until 2026-09-08 — see the "A failed descent is atomic" invariant in `docs/algorithms/leapfrog-triejoin.md`.
...
1. Classify the optimization by the shape/value/process rule (see
   [`docs/specs/optimization-standard.md`](docs/specs/optimization-standard.md)
   § "How to classify"): **Layout** = a shape (which variants, encodings, or
   hash functions exist — zero cost for non-users, by monomorphisation),
   **Config** = a value read on a path the code already takes (replacing a
   constant adds no branch), **BuildMode** = a process yielding the same
   shape. The test is the non-user tax: if a run that leaves the
   optimization off would pay anything for its existence, it is a shape.
2. Define the relevant Rust types implementing `LayoutOption`/`ConfigOption`/
   `BuildMode` from `kermit_iters::optimization`. A Config on a data structure
   also implements `ConfigurableRelation` (`kermit-ds/src/relation.rs`) so the
   value can reach the constructor; `Configured<R, P>`
   (`kermit-ds/src/configured.rs`) lifts it to a type for the test macros.
...
- **JSON bench reports**: every `kermit bench` invocation writes a machine-readable report. Default path is `bench-runs/{kind}-{unix-millis}.json` (`bench-runs/` is auto-created and gitignored at the workspace root); `--report-json <PATH>` overrides. Output is always a JSON array of `BenchReport` objects (one per query for `bench run`, exactly one for `bench join` / `bench ds`). Each object carries `metadata` (label/value pairs mirroring stderr), `axes` (a `BTreeMap<String, serde_json::Value>` of structured axis values for tooling — conventional keys: `data_structure`, `algorithm`, `optimiser`, `query`, `benchmark`, `relation_path`, `relation_bytes`, `tuples`, `arity`), and `criterion_groups` pointers resolving to `target/criterion/{group}/{directory_name}/`. Never compute the on-disk directory names: Criterion 0.8.2 escapes `/` to `_`, **truncates each name to 64 bytes**, and suffixes `_2`, `_3`… on a clash within one `Criterion` instance — resolve through each `new/benchmark.json`'s `(group_id, function_id)` (`kermit_lab.criterion.CriterionIndex`). Because `bench run` builds a fresh `Criterion` per group, two groups sharing their first 64 bytes would overwrite each other, so `check_sweep_group_directories` (`kermit/src/bench/run.rs`) refuses such a sweep up front; its `criterion_directory_name` copy of Criterion's private naming function is pinned against the real crate by a test, which will flag a Criterion upgrade that changes the rule. Schema is versioned via `schema_version` (currently `2`) and lives in `kermit/src/bench_report.rs`; full key catalogue in `docs/specs/bench-report-schema.md`. Bump the version on any breaking field-name or value-type change. `verified: true` appears in `axes` only when `bench run --verify` checked the query. Python's `SCHEMA_VERSION` (`python/kermit-lab/kermit_lab/__init__.py`) is pinned to `REPORT_SCHEMA_VERSION` by a `cargo test` that embeds the Python file with `include_str!`; bump both together. `bench run` writes the report incrementally through `ReportSink`, rewriting it atomically after every cell, so an interrupted sweep keeps the reports of every cell that finished; the failing cell's error names the retained file.
- **Workspace root is resolved at runtime**: `materialize::workspace_root` honours a `KERMIT_WORKSPACE` env override, then walks up from the current directory to the first `Cargo.toml` containing a `[workspace]` table, and only then falls back to the compile-time crate location with a one-line stderr note (printed once per process). So a binary built in one worktree and run from another reads the *running* tree's `benchmarks/`, matching where `bench-runs/` and `target/criterion/` land.
- **bench `--name` semantics**: For `bench ds`, `--name` is the full Criterion group name (default `ds`). For `bench run` and `bench join` it is a *prefix* on the auto-generated `{workload}/{query}/{ds}/{algo}` identity (defaults `run` / `join`; `bench join`'s workload is `adhoc` and its query is the query file's stem), so workload identity stays in `target/criterion/{group}/`.
- **`bench run` sweeps are cells, not pairs**: `kermit/src/execution.rs` defines `Execution` — an enum whose variants *are* the three valid (structure, algorithm) cells (`TrieLftj(TreeTrie|ColumnTrie)`, `HashHtj { hasher, pruning, config }`). `Sweep::expand` turns the `-i`/`-a` selectors into cells and a `skipped` list; `-i all -a all` runs exactly the 3 valid cells (skips announced on stderr), and a single explicit incompatible pair is a usage error. One generic `run_benchmark<F: ExecutionFamily>` in `kermit/src/bench/run.rs` serves both trait families and both `bench run` and `bench join`, fed by a `bench::Workload` (`kermit/src/bench/workload.rs`: `from_definition` for YAML benchmarks, `adhoc` for `bench join`) and a `RunSettings`; the report's `data_structure`/`algorithm` axes come from `F::execution()`, so a report cannot name an algorithm it did not run (issue #56). `bench ds` uses the same shape: `Execution::for_structure` (total, since each structure has one compatible algorithm) picks the cell and one generic `run_ds_bench<F: RelationFamily>` in `kermit/src/bench/ds.rs` serves both (issue #61). `RelationFamily` is the relation-facing supertrait of `ExecutionFamily`; `bench ds` uses the structure-only markers `SortedTrieFamily<R>` / `HashTrieFamily<H, P>`, which cannot join, and the join families embed and delegate to them. Every relation built from a tuple snapshot goes through `RelationFamily::build_relation` (and `RelationFamily::load` on top of it), the one config-aware construction site.
...
- **CI env vars**: All CI jobs set `RUST_BACKTRACE=1`.
- **Not published to crates.io**: `publish = false` is set in `[workspace.package]` and inherited by every crate (`publish.workspace = true`). `release.yml` creates a `v<kermit version>` tag and a git-cliff GitHub release whenever `kermit/Cargo.toml`'s version has no release yet — nothing more. The crates.io copies (≤ June 2026) are stale; don't treat them as a baseline.
...
- **Selection rewrite (repeated variables)**: the same body then calls `kermit_algos::rewrite_repeated_variables`. A variable occurring twice in one body atom (`r(X, X)`) is a selection `σ_{col_i = col_j}`, not something the executors handle — both register a predicate once per variable, so before the rewrite LFTJ silently returned every first-column key and HashTriejoin panicked at the leaf. Each later occurrence becomes a fresh `K<n>` variable (shared counter with the const rewrite; const runs first) and the atom is renamed `Select_<n>_<base>`, backed via `JoinFamily::wrap_selection` by `EqualitySelectionTrieIter` / `EqualitySelectionHashTrieIter` (`kermit-algos/src/{sorted,hash}/selection.rs`), which admit only the source level's key at the repeat level (the hash view also value-filters its leaf chain). A new `JoinAlgo` impl must tolerate `Select_*` predicates like any other relation; a new family needs its own `selection.rs`. Head repeats (`Q(X, X)`) and non-trailing placeholders are out of scope. Neither entry point projects to the head, so the fresh column is emitted alongside the head columns exactly as const-rewrite variables already are.
...
- **Config values are build-time for `HashTrie`**: the one Config value is `HashTrieConfig::load_factor` (`--ds-config load-factor=0.5`, axis `ds_config_load_factor`, default 0.7), read only while the trie is built — `HashTrie::insert_at` passes it into the resize test in `HashTable::entry_or_insert_with`, which is the path that makes it a Config rather than a Layout (a value substituted for a constant, no new branch). So a config must reach the constructor (`from_tuples_with_config`); there is no setter. `bench ds` therefore loads tuples via `kermit_ds::read_csv` / `read_parquet` and builds once with the config, rather than `from_csv` then rebuilding. `bench run` does the same through `ExecutionFamily::load` / `ExecutionFamily::build_relation` (`kermit/src/execution.rs`) — the seam itself is unchanged by the pruning reclassification, which only added the `pruning` field to `Execution::HashHtj` and the `P` parameter to `HashHtj<H, P>`: `load`'s default reads the tuples and hands them to `build_relation`, so each family has exactly one config-aware construction site and the per-relation `insertion` metric cannot time an unconfigured build under configured `ds_config_*` axes.
- **Singleton pruning is a Layout, not a Config**: it is the `P` parameter of `HashTrie<H, P>` (`--ds-layout-pruning off|on`, axis `ds_layout_pruning`), because it changes the *shape* — `HashTrieNode::Singleton(P::Payload)` — and a Config encoding made pruning-*off* runs pay ~10 % on join iteration. Under `NoPruning` the payload (and the iterator's singleton frame) is the uninhabited `Never`, so the variant cannot be constructed and the off instantiation compiles back to the pre-pruning code (pinned by `node_does_not_grow_under_the_pruning_policy` and `off_frame_is_the_bare_table_pair`). The hasher × pruning product is expanded in exactly one place, the `with_hash_trie_layout!` macro in `kermit/src/options.rs`, which `dispatch_run_bench` (`kermit/src/bench/run.rs`), `dispatch_ds_bench` (`kermit/src/bench/ds.rs`) and `load_query_runner` (`main.rs`) all call — a third Layout dimension means new arms there and nowhere else. History and the measurements: `docs/specs/2026-09-08-singleton-pruning-config-design.md` § Amendment 1.
- **Relation integrity**: `RelationSource.sha256` is optional; `ensure_cached` checks a declared digest against the downloaded bytes before anything reaches the cache path, and `bench fetch` re-hashes cached and committed files via `cache::verify_integrity`; `bench run` never hashes.
