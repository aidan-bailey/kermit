...
def _term_to_datalog(term, uri_to_id: dict[str, int]) -> tuple[str, str | None]:
    ...
    Mutates ``uri_to_id`` in place: URI constants not seen in the source
    data (e.g. WatDiv stress-1000 queries reference Offer IDs absent
    from the 10M dataset) are assigned a fresh dictionary ID rather
    than raising. SPARQL semantics treat these as a no-match; the
    kermit engine reaches the same outcome because the synthetic
    ``c<id>`` never appears in any predicate Parquet, so the singleton
    trie iterator generated by ``kermit-algos/src/const_rewrite.rs``
    joins to nothing. Callers are expected to re-emit dict artifacts
    (see :func:`watdiv_preprocess.pipeline.run_pipeline`) if they
    observe that the dict grew during translation.
    """
